...
// 2. Marcador exacto = 5 puntos
$exact = ScoringService::points('exact', 2, 1, null, 2, 1);
if ($exact['points'] === 5 && $exact['reason'] === 'exact_score') {
    line('[OK] Marcador exacto acertado otorga 5 puntos');
    $ok++;
} else {
    line('[ERROR] Marcador exacto debería dar 5 pts, obtuvo: ' . json_encode($exact));
    $errors++;
}

// 3. Props O/U — matriz original
try {
    PropPrediction::assertUniqueOuMatrices();
    line('[OK] Matrices O/U válidas (Más fácil=1, Menos difícil en línea baja)');
    $ok++;
} catch (\RuntimeException $e) {
    line('[ERROR] Matrices O/U: ' . $e->getMessage());
    $errors++;
}

$goalsOverLow = PropPrediction::pointsForMarket('goals_ou', 0.5, 'over');
$goalsOverMid = PropPrediction::pointsForMarket('goals_ou', 1.5, 'over');
if ($goalsOverLow === 1 && $goalsOverMid === 1) {
    line('[OK] Goles más 0.5 y 1.5 = 1 pt');
    $ok++;
} else {
    line("[ERROR] Goles más 0.5/1.5 esperado 1/1, obtuvo {$goalsOverLow}/{$goalsOverMid}");
    $errors++;
}
...
$goalsUnderHigh = PropPrediction::pointsForMarket('goals_ou', 4.5, 'under');
$goalsUnderMid = PropPrediction::pointsForMarket('goals_ou', 3.5, 'under');
if ($goalsUnderHigh === 1 && $goalsUnderMid === 1) {
    line('[OK] Goles menos 3.5 y 4.5 = 1 pt');
    $ok++;
} else {
    line("[ERROR] Goles menos 3.5/4.5 esperado 1/1, obtuvo {$goalsUnderMid}/{$goalsUnderHigh}");
    $errors++;
}

$cornersOverLow = PropPrediction::pointsForMarket('corners_ou', 8.5, 'over');
$cornersUnderLow = PropPrediction::pointsForMarket('corners_ou', 8.5, 'under');
if ($cornersOverLow === 1 && $cornersUnderLow === 4) {
    line('[OK] Córners más 8.5 = 1 pt, menos 8.5 = 4 pts');
    $ok++;
} else {
    line("[ERROR] Córners 8.5 esperado más 1 / menos 4, obtuvo {$cornersOverLow}/{$cornersUnderLow}");
    $errors++;
}

$cardsOverLow = PropPrediction::pointsForMarket('cards_ou', 2.5, 'over');
$cardsUnderLow = PropPrediction::pointsForMarket('cards_ou', 2.5, 'under');
if ($cardsOverLow === 1 && $cardsUnderLow === 4) {
    line('[OK] Tarjetas más 2.5 = 1 pt, menos 2.5 = 4 pts');
    $ok++;
} else {
    line("[ERROR] Tarjetas 2.5 esperado más 1 / menos 4, obtuvo {$cardsOverLow}/{$cardsUnderLow}");
    $errors++;
}

// 4. user_points vs sum ledger
line('');
...
// 7. Ledger alineado con config actual (tabla de posiciones lee points_ledger)
line('');
line('--- Ledger vs puntuación actual (leaderboard) ---');
$cfg = require dirname(__DIR__) . '/app/Config/app.php';
$exactExpected = (int)($cfg['exact_score_points'] ?? 5);

$staleExactSt = $pdo->prepare(
    "SELECT COUNT(*) FROM points_ledger WHERE reason = 'exact_score' AND points > 0 AND points != :expected"
);
$staleExactSt->execute(['expected' => $exactExpected]);
$staleExact = (int)$staleExactSt->fetchColumn();
if ($staleExact > 0) {
    line("[ERROR] {$staleExact} acierto(s) exact_score en ledger con puntos distintos de {$exactExpected} — requiere re-liquidación");
    $errors++;
} else {
    line("[OK] Aciertos exact_score en ledger usan {$exactExpected} pts (o no hay aciertos)");
    $ok++;
}

$ouSt = $pdo->query(
    "SELECT pl.id, pl.points, pp.market, pp.line, pp.pick
     FROM points_ledger pl
     INNER JOIN prop_predictions pp ON pp.id = pl.prop_prediction_id
     WHERE pl.points > 0 AND pl.reason IN ('correct_goals_ou','correct_corners_ou','correct_cards_ou')"
);
$staleOu = 0;
foreach ($ouSt->fetchAll() ?: [] as $ouRow) {
    $expectedPts = PropPrediction::pointsForMarket(
        (string)$ouRow['market'],
        (float)$ouRow['line'],
        (string)$ouRow['pick'],
    );
    if ((int)$ouRow['points'] !== $expectedPts) {
        line("[ERROR] Ledger O/U #{$ouRow['id']}: {$ouRow['points']} pts, config actual={$expectedPts} "
            . "({$ouRow['market']} {$ouRow['line']} {$ouRow['pick']})");
        $errors++;
        $staleOu++;
    }
}
if ($staleOu === 0) {
    line('[OK] Props O/U liquidados coinciden con matriz actual');
    $ok++;
}

$boardWithDetail = UserPoints::attachLastMatchBreakdown($boardRows, $season);
$detailChecked = 0;
foreach ($boardWithDetail as $detailRow) {
    $last = $detailRow['last_match'] ?? null;
    if (!is_array($last) || empty($last['is_settled']) || empty($last['columns'])) {
        continue;
    }
    $detailChecked++;
    $detailTotal = (int)$last['total'];
    $uid = (int)$detailRow['id'];
    $mid = (int)$last['match_id'];
    $sumSt = $pdo->prepare(
        'SELECT COALESCE(SUM(points), 0) FROM points_ledger WHERE user_id = :uid AND match_id = :mid'
    );
    $sumSt->execute(['uid' => $uid, 'mid' => $mid]);
    $ledgerMatchSum = (int)$sumSt->fetchColumn();
    if ($detailTotal !== $ledgerMatchSum) {
        line("[ERROR] {$detailRow['name']}: desplegable último partido total={$detailTotal} vs ledger={$ledgerMatchSum}");
        $errors++;
    } else {
        line("[OK] {$detailRow['name']}: desplegable último partido = {$detailTotal} pts (ledger)", true);
        $ok++;
    }
}
if ($detailChecked === 0) {
    line('[INFO] Sin partidos liquidados con desplegable para contrastar detalle', true);
}
