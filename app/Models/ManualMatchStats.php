...
final class ManualMatchStats
{
    ...
    public static function upsert(
        ...
        if ($totalCorners === null && $totalCards === null && $totalGoals === null && $btts === null) {
            throw new \RuntimeException('Ingresa al menos córners o tarjetas (amarillas/rojas).');
        }
    ...
    public static function hasYellowCards(?array $stats): bool
    {
        return $stats !== null
            && array_key_exists('total_yellow_cards', $stats)
            && $stats['total_yellow_cards'] !== null;
    }

    /** @return list<array<string, mixed>> */
    public static function finishedMatchesPendingCornersAndYellow(int $season): array
    {
        $st = DB::pdo()->prepare(
            "SELECT m.id, m.kickoff_at, m.status, m.home_score, m.away_score,
        ...
        foreach ($rows as $row) {
            $matchId = (int)$row['id'];
            $stats = MatchStats::forMatch($matchId);
            if (MatchStats::hasStatsForMarket($stats, 'corners_ou')) {
                continue;
            }
            if (self::hasYellowCards($stats)) {
                continue;
            }
            $pending[] = $row;
        }
