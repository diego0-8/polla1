...
final class Team
{
    public static function upsertFromApi(
        ...
        $name = TeamName::toSpanish($apiTeamId, $code, $name);
    ...
    public static function forTournamentSeason(int $season): array
    {
        ...
        foreach ($rows as $idx => $row) {
            $rows[$idx] = TeamName::applyToTeamField($row, 'name', 'code', 'api_team_id');
        }
    ...
    public static function findById(int $id): ?array
    {
        ...
        $st->execute(['id' => $id]);
        $row = $st->fetch();
        return $row ? TeamName::applyToTeamField($row, 'name', 'code', 'api_team_id') : null;
    }
