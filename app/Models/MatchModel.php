...
final class MatchModel
{
    ...
    private const TEAM_JOIN_SELECT = '
        th.name AS home_name, th.code AS home_code, th.logo_url AS home_logo, th.api_team_id AS home_api_team_id,
        ta.name AS away_name, ta.code AS away_code, ta.logo_url AS away_logo, ta.api_team_id AS away_api_team_id';
    ...
    public static function findById(int $id): ?array
    {
        ...
        return $row ? TeamName::applyToMatch($row) : null;
    }

    public static function today(): array
    {
        $st = DB::pdo()->prepare(
            'SELECT m.*,' . self::TEAM_JOIN_SELECT . '
             FROM matches m
             JOIN teams th ON th.id = m.home_team_id
             JOIN teams ta ON ta.id = m.away_team_id
    ...
    /** @return array{0:string,1:string} Límites [desde inclusive, hasta exclusive] para kickoff_at */
    public static function seasonKickoffBounds(?int $year = null): array
    {
    ...
    {
        $year = $year ?? self::seasonYear();
        [$from, $to] = self::seasonKickoffBounds($year);
        $st = DB::pdo()->prepare(
            'SELECT m.*,' . self::TEAM_JOIN_SELECT . '
             FROM matches m
             JOIN teams th ON th.id = m.home_team_id
    ...
    public static function upcomingForSeason(int $limit = 10, ?int $year = null): array
    {
        ...
        $st = DB::pdo()->prepare(
            'SELECT m.*,' . self::TEAM_JOIN_SELECT . '
             ...
             JOIN teams th ON th.id = m.home_team_id
             JOIN teams ta ON ta.id = m.away_team_id
             WHERE m.kickoff_at >= :now AND m.kickoff_at < :to
             ORDER BY m.kickoff_at ASC
             LIMIT :lim'
        );
        ...
        $rows = $st->fetchAll() ?: [];
        if ($rows !== []) {
            return TeamName::applyToMatches($rows);
        }
    ...
    public static function countFinishedForSeason(?int $year = null): int
    {
        $year = $year ?? self::seasonYear();
        [$from, $to] = self::seasonKickoffBounds($year);
        $st = DB::pdo()->prepare(
