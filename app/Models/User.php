...
final class User
{
    ...
    public const USERNAME_PATTERN = '/^[a-z0-9._]{3,30}$/';
    ...
    public static function assertValidUsername(string $username): void
    {
        if (!preg_match(self::USERNAME_PATTERN, $username)) {
            throw new \RuntimeException('Usuario: 3-30 caracteres; letras, números, punto (.) y guion bajo (_).');
        }
    }
    ...
    public static function updateProfile(
        ...
        self::assertValidUsername($username);
    ...
    public static function suspendUnpaidAsesorsPastDeadline(): int
    {
        $st = DB::pdo()->prepare(
            'UPDATE users u
             INNER JOIN user_roles ur ON ur.user_id = u.id
             INNER JOIN roles r ON r.id = ur.role_id AND r.name = \'asesor\'
             SET u.status = \'suspended\', u.updated_at = NOW()
             WHERE COALESCE(u.has_paid, 0) = 0 AND u.status = \'active\''
        );
        $st->execute();
