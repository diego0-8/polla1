...
final class ManualMatchUpdate
{
    ...
    /** Publica todos los overrides manuales pendientes antes de liquidar puntos. */
    public static function publishAllPending(): void
    {
        self::ensureTables();
        $st = DB::pdo()->query('SELECT match_id FROM manual_match_updates');
        foreach ($st->fetchAll() ?: [] as $row) {
            self::publishToMatchIfApiIncomplete((int)$row['match_id']);
        }
    }
    ...
    public static function applyToMatches(array $matches, bool $resolveEventsSource = false): array
    {
        if ($matches === []) {
            return [];
        }
        ...
        self::ensureTables();
        $matchIds = array_map(static fn (array $m): int => (int)$m['id'], $matches);
        $manualById = self::loadManualByMatchIds($matchIds);
        $manualEventIds = $resolveEventsSource ? self::loadManualEventMatchIds($matchIds) : [];

        foreach ($matches as $idx => $match) {
            $matchId = (int)$match['id'];
            $manual = $manualById[$matchId] ?? null;

            if ($manual === null) {
                $match['data_source'] = 'api';
                if ($resolveEventsSource) {
                    $match['events_source'] = isset($manualEventIds[$matchId]) ? 'manual' : 'api';
                }
                $matches[$idx] = $match;
                continue;
            }

            if (!self::apiHasScoreOrStatus($match, [])) {
                $match['home_score'] = (int)$manual['home_score'];
                $match['away_score'] = (int)$manual['away_score'];
                $match['status'] = (string)$manual['status'];
                $match['manual_note'] = $manual['note'];
    ...
    /** @param list<int> $matchIds @return array<int, array<string, mixed>> */
    private static function loadManualByMatchIds(array $matchIds): array
    {
        if ($matchIds === []) {
            return [];
        }
        ...
        $st = DB::pdo()->prepare("SELECT * FROM manual_match_updates WHERE match_id IN ($ph)");
        $st->execute($matchIds);

        $result = [];
        foreach ($st->fetchAll() ?: [] as $row) {
            $result[(int)$row['match_id']] = $row;
        }
    ...
    private static function loadManualEventMatchIds(array $matchIds): array
    {
        if ($matchIds === []) {
            return [];
        }
        ...
        $ph = implode(',', array_fill(0, count($matchIds), '?'));
        $st = DB::pdo()->prepare("SELECT DISTINCT match_id FROM manual_match_events WHERE match_id IN ($ph)");
        $st->execute($matchIds);
