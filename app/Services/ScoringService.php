...
final class ScoringService
{
    ...
    public static function points(
        ...
        if ($outcome === 'D') {
            return [
                'points' => self::configInt('outcome_draw_points', 3),
                'reason' => 'correct_draw',
            ];
        }

        return [
            'points' => self::configInt('outcome_winner_points', 3),
    ...
    public static function propPoints(string $market, ?float $line, string $pick, array $stats): array
    {
        ...
        return [
            'points' => \App\Models\PropPrediction::pointsForMarket($market, $line, $pick),
            'reason' => $reason,
        ];
    }
