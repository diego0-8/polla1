...
final class FootballDataSyncService
{
    ...
    public function syncLive(): array
    {
        ...
        $window = $this->syncKickoffWindow(5, max(0, $detailBudget));
    ...
    private static function kickoffRecentlyStarted(string $kickoffAt): bool
    {
        try {
            ...
            return $kickoff <= $now && $kickoff >= $now->modify('-5 hours');
        } catch (\Throwable) {
            return false;
        }
    }
     ...
     */
    public function syncKickoffWindow(int $hoursBack = 5, int $maxDetailRequests = 8): array
    {
        $pdo = DB::pdo();
